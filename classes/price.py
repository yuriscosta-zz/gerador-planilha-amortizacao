...
class Price:
    ...
    def _gerar_valores(self):
        ...
        taxa_unitaria = self._taxa_juros / 100
        prestacao = self._montante * (taxa_unitaria / (1 - (1 + taxa_unitaria)**-self._parcelas))
        
        for i in range(0, self._parcelas+1):
            self._lista_periodos.append(i)
            if i != 0:
                juros = self._montante * taxa_unitaria
                self._amortizacao = prestacao - juros
